...
function buildHtml(bike, destination, radiusMeters) {
...
<script>
  ...
  // Pulsing halo around the partner's live position, like Google Maps' live-location
  // sharing — makes the dot easy to spot and gives a visual "how close" sense of scale.
  L.circle(bike, {radius: ${radiusMeters}, color: '#2f6fed', weight: 1, fillColor: '#2f6fed', fillOpacity: 0.15}).addTo(map);
...
// radiusMeters: size of the live-location halo drawn around the bike (default 120m).
export default function BikeMap({bike, destination, height = 200, scrollEnabled = false, radiusMeters = 120}) {
  if (!bike?.lat) return null;
  return (
    <View style={[styles.wrap, {height}]}>
      <WebView
        source={{html: buildHtml(bike, destination, radiusMeters)}}
        style={styles.web}
